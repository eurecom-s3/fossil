...
class MemoryObject:
    @classmethod
    def prepare(cls, dump_ptrs: List, ptr_size: int, v2o, btm, strs, xrefs, functions, dumpfile):
        cls.ptrs = dump_ptrs
        cls.ptrs_sorted = sorted(dump_ptrs.keys())
        cls.ptrs_set = set(cls.ptrs_sorted)
        
        cls.autoptrs_sorted = sorted((k for k,v in cls.ptrs.items() if k == v))
        cls.autoptrs_set = set(cls.autoptrs_sorted)

        cls.strs = strs
        cls.strs_sorted = sorted(strs.keys())
        cls.strs_set = set(strs.keys())


        cls.ptr_size = ptr_size
        cls.v2o = v2o
        cls.btm = btm

        if ptr_size == 4:
            cls.convf = lambda self, x: ctypes.c_uint32(x).value
            cls.dtype = np.uint32
        else:
            cls.convf = lambda self, x: ctypes.c_uint64(x).value
            cls.dtype = np.uint64

        cls.xrefs = xrefs
        cls.xref_referenced = set()

        cls.dumpfile = open(dumpfile, "rb")
        cls.functions = functions

    @classmethod
    def ptrs_is_null(cls, ptr):
        elf_off = cls.v2o[ptr]
        if elf_off == -1 or cls.btm[elf_off:elf_off + cls.ptr_size].any():
            return False
        else:
            return True
...
class PointersGroup(MemoryObject):
    def __init__(self, ptrs_list, offsets=tuple()):
        ...
        xrefs = self.xrefs.intersection(self.ptrs_list)
        self.xref_referenced.update(xrefs)
    ...
    def get_u32(self, offset):
        self.dumpfile.seek(offset, 0)
        try:
            return unpack("<I", self.dumpfile.read(4))[0]
        except:
            return None
    ...
    def find_timestamps(self, min_time, max_time=time.time_ns()):
        ...
        for offset in range(self.shape[0], self.shape[0] + self.shape[1]):
            for ptr in self.ptrs_list:
                file_offset = self.v2o[ptr + offset]
                if file_offset == -1:
                    continue
    ...
    def determine_shape(self, max_offset: int=8192, near_threshold: float=0.9, autostruct_threshold: float=0.9, align_threshold: float=0.5, fake=False):
        ...
        self.list_child_offsets = {offset for offset in self.valid_near_offsets if 0 < len(self.near_ptrs[offset][0].intersection(self.autoptrs_set)) < len(self.near_ptrs[offset][0]) }
    ...
    def find_near_pointers(self, max_offset: int=8192, threshold: float=0.9):
        ...
        for base_ptr in base_ptrs:
            ...
            min_idx = bisect_right(self.ptrs_sorted, base_ptr - mode_distance, lo=min_idx)
            
            for idx in range(min_idx, len_dump_ptrs):
                diff = self.ptrs_sorted[idx] - base_ptr

                if diff >= mode_distance:
                    break
                
                visited_ptrs[diff].append(self.ptrs_sorted[idx])
        ...
        for offset in visited_ptrs.keys():
            for base_ptr in base_ptrs:
                ...
                ptr = base_ptr + offset
                if ptr in self.ptrs_set or \
                (elf_offset := self.v2o[ptr]) == -1 or \
                self.btm[elf_offset:elf_offset+self.ptr_size].any():
                    continue
    ...
    def identify_struct_start(self, max_offset=8192, threshold=0.9):
        ...
        for offset, (ptrs_s, _) in self.near_ptrs.items():
            ...
            # of the cases)
            dests = [self.ptrs[x] for x in ptrs_s if x not in self.autoptrs_set]
            if not dests: # Ignore offsets containing only autopointers
                continue
            minimum_distances = [self.convf( np.min(base_ptrs_array - x) ) for x in dests] # Collect for each pointer the minimum distance between its destination and all the base pointers
            ...
            if count < min(threshold * len(ptrs_s), len(ptrs_s) - 1) or \
                distance >= max_offset or -distance > offset or not (-distance <= offset <= -distance + self.mode_distance - self.ptr_size): # Ignore too big distances
                continue
        ...
        if possible_start > 0 or  not (possible_start  <= min(self.structural_offsets) <= max(self.structural_offsets) <= possible_start + self.mode_distance - self.ptr_size) or \
           any([not (possible_start  <= x <= possible_start + self.mode_distance - self.ptr_size) for x in near_ptrs_same_type]):
    ...
    def estimate_align(self, threshold=0.5):
        ...
        offsets = sorted([k for k,t in self.near_ptrs.items() \
            if (abs(k-max(self.structural_offsets) - self.ptr_size) <= self.mode_distance and \
        ...
        for idx in range(0, idx_final):
            total = 0
            offsets_counter = 0
            limit = offsets[idx] + self.mode_distance - self.ptr_size
    ...
    def find_strings(self, threshold=0.9):
        ...
        for elem in self.ptrs_list:
            ...
            for offset in self.valid_near_offsets:
                ...
                addr = elem + offset
                if addr in self.autoptrs_set:
                    continue

                try:
                    pointed_addr = self.ptrs[addr]
                    if pointed_addr in self.strs:
            ...
            min_idx = bisect_right(self.strs_sorted, minimum_address)
        
            for idx in range(min_idx, len_strs):
                diff = self.strs_sorted[idx] - elem
    ...
    def find_similar_string(self, string):
        ...
        for offset, s_addresses in self.embedded_strs.items():
            for s_address in s_addresses:
                if string in (r := self.strs[s_address]):
    ...
    def extract_strings(self, offset, is_pointed):
        if is_pointed:
            return [self.strs[x] for x in self.pointed_strs[offset]]
        else:
            return [self.strs[x] for x in self.embedded_strs[offset]]
...
class LinkedList(PointersGroup):
    ...
    def termination_status(self):
        ...
        """
        if self.ptrs_list[-1] in self.autoptrs_set:
            return True

        try:
            deref = self.ptrs[self.ptrs_list[-1]] + self.dests_offsets
            if deref  == self.ptrs_list[-1]:
                return True

            elf_off = self.v2o[deref]
            if elf_off == -1 or self.btm[elf_off:elf_off + self.ptr_size].any():
                return False
        except:
            pass

        elf_off = self.v2o[self.ptrs_list[-1]]
        if elf_off == -1 or self.btm[elf_off:elf_off + self.ptr_size].any():
            return False
        else:
            return True
...
class Tree(PointersGroup):
    ...
    def get_tree_pointed_strs(self, offset):
        ...
        for ptr in self.nodes:
            try:
                if ptr and self.ptrs[ptr + offset] in self.pointed_strs[offset]:
                    tree_list.append(self.ptrs[ptr + offset])
                else:
                    tree_list.append(None)
...
class PtrsArray(MemoryObject):
    def __init__(self, ptrs_list):
        ...
        dest_ptrs = {self.ptrs[x] for x in ptrs_list}
        self.referenced = bool(self.xrefs.intersection(self.ptrs_list))
        self.structs = None
        self.referenced_fn = bool(self.functions.intersection(self.ptrs_list))

        # Find if all the pointers point directly to strings
        if dest_ptrs.issubset(self.strs_set):
